...
| Haskell           | `.hs`, `.lhs`                                   | tree-sitter-haskell             | function, type (data/newtype/type synonym), class (class/instance), method                 | —              | `--` and `{- -}` comments     | Custom parser (#722): signature and pattern clauses merge into one function; `where` locals and operator definitions are not indexed |
