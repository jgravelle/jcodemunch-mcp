...
| ----------------- | ----------------------------------------------- | ----------------------------- | ------------------------------------------------------------------------------------------ | -------------- | ----------------------------- | ------------------------------------------------------------------------------------------- |
| Python            | `.py`                                           | tree-sitter-python            | function, class, method, constant, type, field                                             | `@decorator`   | Triple-quoted strings         | Type aliases require Python 3.12+ syntax; class-body state is indexed in every class (#784): UPPER_CASE as `constant`, otherwise `field`; dunders are not |
| JavaScript        | `.js`, `.mjs`, `.cjs`, `.jsx`                   | tree-sitter-javascript        | function, class, method, field, constant                                                   | —              | `//` and `/** */` comments    | Anonymous arrow functions without assigned names are not indexed                            |
| TypeScript        | `.ts`                                           | tree-sitter-typescript        | function, class, method, field, constant, type                                             | `@decorator`   | `//` and `/** */` comments    | Decorator extraction depends on Stage-3 decorator syntax                                    |
| TSX               | `.tsx`                                          | tree-sitter-tsx               | function, class, method, field, constant, type (interface/enum/alias)                      | `@decorator`   | `//` and `/** */` comments    | JSX-aware TypeScript; separate grammar from `.ts`                                           |
...
| C++               | `.cpp`, `.cc`, `.cxx`, `.hpp`, `.hh`, `.hxx`, `.h`* | tree-sitter-cpp           | function, class, method, field, type (struct/enum/union/alias), constant                  | —              | `/* */` and `//` comments     | Namespace symbols used for qualification but not emitted as standalone                      |
...
| Arduino           | `.ino`, `.pde`                                  | tree-sitter-arduino             | function, class, method, field, type (struct/enum/union), constant                         | —              | `/* */` and `//` comments     | C++ superset grammar; `.ino`/`.pde` sketches; `#define` macros as constants                 |
...
| Haskell           | `.hs`, `.lhs`                                   | tree-sitter-haskell             | function, type (data/newtype/type synonym), class (class/instance), method                 | —              | `--` and `{- -}` comments     | Custom parser (#722): signature and pattern clauses merge into one function; literate `.lhs` is read in both styles; `where` locals and infix operator definitions are not indexed |
