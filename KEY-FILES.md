...
src/jcodemunch_mcp/
  ...
  tools/
    ...
    get_tectonic_map.py       # get_tectonic_map: logical module topology via 3-signal fusion (structural+behavioral+temporal) + Louvain modularity clustering (label propagation until #668)
  ...
  runtime/
    ...
    diagnostics_log.py   # (2026-09) Pure parsers for a checker's OWN output: mypy `--output json` (JSON-Lines), pyright `--outputjson` (0-based lines, converted here), tsc `--pretty false` (text `path(line,col): error TSnnnn: msg`), ruff `--output-format json` (array; every row a `warning`), generic JSON-Lines `{file,line,severity,message,code?,tool?}`. `detect_format(text)` decides by CONTENT and raises rather than guesses; every parser yields the same 1-based `Diagnostic`.
  ...
  tools/
    _diagnostics_consume.py  # (2026-09) Shared reader for the `diagnostics` snapshot (the `_scip_consume` pattern): `load_symbol_diagnostics(db_path, ids)` (None = NO DATA; a symbol with data but no rows gets a real zero), `diagnostics_snapshot(db_path)` ({as_of, ingested_at, tools}), `diagnostics_currency(as_of, live_head)` (tri-state), `live_git_head(source_root)`, `summarise(entry)` (`mypy arg-type ×2`). Read-only; used by check_edit_safe, get_changed_symbols, get_pr_risk_profile, get_symbol_provenance.
