...
Current: counter 939 tokens vs full_full 22,741 (95.9% avoided); core_compact 3,885 in the baseline, live gate at 3,998 of 4,000 per `CLAUDE.md` (#571); counter surface 6 tools, 4,184 B, byte-pinned; live drift vs baseline at most 4.5% (full_compact) on 2026-09-03. Route@1 measured 71.2% on the human corpus. The route gate is [`route.control_at1`] on the held-out CONTROL subset (Floor line below), and the human corpus is not gated.
Floor: core_compact at or under the [`schema.core_compact_ceiling`] (4,000 at set time); every profile within [`schema.drift_tolerance`] of the committed baseline; counter surface byte-identical to its pin; counter avoids at least [`counter.saving_min`] of `full_full`; **route@1 on the held-out CONTROL subset at or above its pre-fix baseline [`route.control_at1`, floor 40.0, target 55.0]. CORRECTION 2026-09-03 (FINDINGS F-02): the earlier "route@1 >= 60%" was never a gate, and 55% is the moratorium EXIT bar, the target of `route.control_at1`.**
...
3. For a change under `src/`: a `CHANGELOG.md` entry under `[Unreleased]` (or in a new version block, the release commit's case) that states what was wrong, why, and what the fix makes impossible, or the `no-changelog` label with the reason in the PR. A change with nothing under `src/` owes no entry. `scripts/dod_changelog.py` is the gate and reads this rule and nothing else; the harness, workflows, inbound and competitive layers record their changes in their own FINDINGS files by that layer's convention, which no gate reads.
