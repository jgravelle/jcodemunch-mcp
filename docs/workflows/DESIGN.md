...
| H5 `deny_guard.py` | `PreToolUse`, `Bash\|PowerShell` | 1 s | Belt to D8's braces: matches the same ACTS as the deny list (`git push --force`, the creating forms of `git tag`, `gh release create|edit|delete|delete-asset|upload`, `gh workflow run`, `gh pr merge`, `gh issue delete`, `twine upload|register`, `mcp-publisher publish|login`, and `gh api` writes whose path is a merge, release, dispatch or tag ref, or whose method is DELETE) and blocks with the RUNBOOK section the human runs. Posting verbs pass (W-40); reads pass too (`gh release view|list|download`, `twine check`, the publisher's `--version`/`--help`, prose that names a binary without its verb; W-44). | Exit 2. |
