...
| L-25 | 2026-09-26, #852 | A later prototype in a list whose parameter is a type name with no declared parameter name, `int f(int), h(Foo);` or `h(const Foo&)`, binds nothing extra: the grammar spells a constructor call (`JsonString a(s1), b(s2);`) the same way, and that shape was the only one on 1,124 real files. Telling them apart needs to know whether `Foo` names a type. | `src/jcodemunch_mcp/parser/extractor.py` `_parameters_could_be_arguments` | gap | low | OPEN |
