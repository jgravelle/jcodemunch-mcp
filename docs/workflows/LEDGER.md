...
| L-04 | 2026-09-25, #759 | With the register's samples, Fortran (`enum, bind(c)`), Julia (`@enum`), Objective-C (`NS_ENUM` and plain `enum`) and SQL (`CREATE TYPE ... AS ENUM`) produce no enum symbol at all, not even the enum. Unverified: the sample may be what fails, so check each grammar's parse before fixing. `NO_ENUM` in `tests/test_enum_members_register.py`. | `src/jcodemunch_mcp/parser/languages.py` (those four specs) | defect | low | OPEN |
