...
| L-22 | 2026-09-26, #850 | Some test leaks an empty `jcm-402-*` directory under `%TEMP%` on every run: 3,505 of them on this box, the newest 2026-09-25. Empty, so no disk or scan cost measured; the leak is the defect. | `tests/test_v1_108_227.py:224-225` (`tempfile.mkdtemp(prefix="jcm-402-")` with no cleanup) | defect | low | OPEN |
...
| L-24 | 2026-09-26, #850 | A C++ class member line naming a field then a method, `struct K { int x, f(int); };`, gives `K.x#field` and no `f` method, as on main. #850's later-prototype rule covers file and block scope only. | `src/jcodemunch_mcp/parser/extractor.py` field channel (`_cpp_declarator_is_function` per member) | defect | low | OPEN |
