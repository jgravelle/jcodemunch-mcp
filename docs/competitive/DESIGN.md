...
| 6 install friction | PARTIALLY. Image build seconds and image size MEASURED per pin in the result header (`image_build_seconds`, `image_size_bytes`, from the D2 build and its `docker image inspect`, 2026-09-09; a header fact, once per adapter per process, not a per-corpus axis). **The prerequisite count is designed, not measured (CF-61)** | the count of prerequisites the Dockerfile had to install beyond the package (a proxy, labelled as one); the handshake and config-parity halves are ours |
