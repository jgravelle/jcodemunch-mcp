...
| **zvec-grep** (zvec-ai/zvec-grep; the org's blog is zvec.org, `company` unset) | ripgrep + BM25 + vector search over a workspace, structure-aware chunking for seven languages ("Symbols, signatures, breadcrumbs, and surrounding source", `docs/04-pipeline.md`), CLI (`zg`) and MCP server, one-command installs into Claude Code, Codex, OpenCode, Qwen Code, Qoder; added 2026-09-07 on jjg's report, facts from `gh api`, `npm view` and the raw README/docs that day | npm `@zvec/zvec-grep` 0.2.2 (2026-09-07), Apache-2.0, Node >=22; 3,124 stars, 168 forks, created 2026-07-10 | v0.2.0 2026-08-27, v0.1.5 2026-07-16; 0.2.2 on npm without a GitHub release | pushed 2026-09-07; 23 open issues (`search/issues?q=repo:zvec-ai/zvec-grep+is:issue+is:open`; the repo's `open_issues_count` of 42 includes PRs) | claims: a 20-task, 11-repo SWE-QA run with Claude Code 2.1.212 + Opus 5, baseline "Claude Code uses its standard tools", scored by an LLM judge ("Judge values are equal-weight means"), with the treatment profiles on `qwen/qwen3.7-text-embedding`, a REMOTE model; the README's prose carries no figure and the results are an SVG image ("comparing answer quality, input tokens, tool calls, and agent time"); it names no other tool; default MCP surface is ONE tool (`zvec_grep_search`), six in full; "Indexed source previews are omitted unless requested"; freshness `fresh` / `possibly_stale` | YES: `--mode direct` runs in-process with no daemon (`auto` never starts one); the local model (`local/potion-code-16m-v2`, the smallest code model) downloads on first use into `ZVEC_GREP_MODEL_CACHE`, so it is pre-warmed into the D2 image the way the tiktoken asset is (harness F-14) and the run stays offline. ⚠ That is NOT the configuration their claim was measured with (remote Qwen); the fairness note records both and the README's recommended local model is what runs |
...
| 9 | zvec-grep 0.2.2 (`ZVEC_GREP_MODE=direct`, `local/potion-code-16m-v2`) | C/D | admitted 2026-09-07 on the §5.4 trigger (3,124 stars, 9.9x the floor, two months old); the member whose published claim is CLOSEST in shape to ours (agent tool calls and input tokens against the agent's own tools, Baseline B's comparison, run by them on Claude Code + Opus 5), but not the same protocol: a model-driven agent scored by an LLM judge, on a remote embedding model the adapter will not use (D1: our loop calls no model; D3: the fairness note says so); a one-tool default MCP surface, the extreme point of the Counter's axis; source of two fix-title probes (FINDINGS CF-66). Adapter: cymbal's subprocess shape with CocoIndex's model warm step |
...
- **On trigger**: a third-party benchmark naming jCodeMunch beside a tool not
  in the set (the sverklo-bench shape), or a tool outside the set, listed in
  §5.3 or not, crossing the adoption floor of the smallest member (cymbal, 317
  stars), opens a `competitive-watch` draft. The draft may admit the tool
  before the quarterly review when §5.1 holds on every condition, as the same
  one PR touching this file and FINDINGS.md (first use: zvec-grep,
  2026-09-07, CF-66; it was on neither list).
