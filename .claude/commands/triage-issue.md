...
1. **Read.** `GITHUB_TOKEN="" gh issue view $ARGUMENTS --comments --json
   ...
   --json number,title,state`. `GITHUB_TOKEN="" gh label list` for the
   label set (never a remembered one).
2. **Classify.** One of: bug, feature, question, duplicate (of #N),
   ...
   request for behaviour that does not exist? For a bug, is it a
   regression of a Standing lesson or an ARCHAEOLOGY row (grep both)?
3. **Split.** Count the findings in the body. More than one: propose one
   ...
   belongs to it, cross-linked, credit on each.
4. **Vendor shape.** If the issue asks for a named provider, gateway, SDK
   or endpoint, run policy 3c's three profile queries (`gh api users/<login>`,
...
5. **Draft the response.** Load `pr-description` (its comment half). A
   ...
   the deadline AND the default action in the same sentence and never
   exceeds 24 hours (policy 3a); no extension clause is advertised. Never
   answer pain with aggregates (3b). Hand the draft to the `spokesperson`
   agent for the outward-bound pass and keep its version.
6. **Write** `TRIAGE.md`: class, labels to add (from step 1's list), split
   proposal, draft response, and the exact `gh` lines the human would run
   to apply them (cmd.exe form). Print it. Post nothing.
