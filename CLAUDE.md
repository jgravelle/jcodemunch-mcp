...
## Current State
- **Version:** 1.108.316 — **Markdown (.md / .markdown) language support.** `.md`/`.markdown` files were dropped at discovery as `wrong_extension`; now indexed via the bundled tree-sitter `markdown` grammar through new `_parse_markdown_symbols` (extractor.py) — registration mirrors Svelte (ext map + `MARKDOWN_SPEC`/LANGUAGE_REGISTRY, auto-flows into the MCP language enum + config validation). **`heading` symbols** (ATX `#`..`######` + setext underlined) parented BY HEADING LEVEL (h2 under nearest preceding lower-level heading — the grammar's own `section` nesting is used only for the byte range: a heading's extent covers its whole section so `get_symbol_source` returns section text; setext headings sit FLAT in the grammar so their extent is cut at the next sibling setext heading). Names emphasis-stripped (`## **Usage**` → `Usage`), empty headings → `(untitled)`, duplicates disambiguated INLINE (`~2`) so parent ids never dangle (the shared post-pass renames ids after children already referenced them). Heading `docstring` = section's first paragraph (skips siblings BEFORE the heading — flat setext sections put the previous heading's paragraph there). **`code_block` symbols** (fenced ``` / ~~~) named by info-string language (`python`; `code` when absent), parented under the enclosing heading. Frontmatter (`minus_metadata`) skipped; no headings/fences → no symbols, file stays text-search-only (SASS precedent); NO import edges from doc text (snippets don't leak into the import graph); `.mdx` deliberately NOT routed (no JSX-aware grammar in the pack). **`VALID_KINDS` += `heading`, `code_block`, and `field`** — the search_symbols `kind=` gate was missing `field` (emitted since v1.108.80 dataclass fields), so `search_symbols(kind="field")` errored; now fixed alongside. **`find_dead_code` doc-file guard**: markdown has no importers by nature — without the skip every indexed `.md` reported `zero_importers` @1.0 with all headings dead. File summaries: `Markdown doc: N sections (...), M code blocks` (no "0 functions" noise). NO INDEX_VERSION bump (additive; re-index gains symbols), no new tool. ⚠⚠ **The schema budget had NO headroom**: upstream's releases since .113 filled live core_compact to the 4,000 ceiling and the new kind enum measured **4,003 — caught by CI's live gate, NOT locally** (the cl100k tokenizer download is network-gated here; local-green was blind). Fix = the existing mechanism: **`kind` joins `language` in `_COMPACT_DEMOTE_ENUM_PARAMS`** — under compact the enum is a free-string filter (the runtime gate answers an unknown with the full valid list IN-BAND, more information than schema rejection), reclaiming ~20 tokens; full surfaces keep both enums inside the 5% drift guard. ⚠⚠ **The schema-derived kind enum is a WIRE change** — check both the runtime gate AND the published schema when a kind is added. New `tests/test_markdown.py` (18) + `tests/fixtures/markdown/sample.md`.
