...
KIND_ORDER: tuple[str, ...] = (
    ...
    # ⚠⚠ (#741, #742) A module-scope MUTABLE binding: JS/TS `let` and `var`.
    # Deliberately NOT `property`, which #732 added for class state -- a
    # top-level `let` belongs to no type, and reusing that kind would mix
    # module bindings into every consumer asking about a class's members. It is
    # the word the grammar itself uses (`variable_declaration`,
    # `variable_declarator`) and the word LSP uses for the same distinction
    # (Variable, separate from both Property and Constant).
    ...
    "variable",   # Module-scope mutable bindings: JS/TS `let`, `var`
)
